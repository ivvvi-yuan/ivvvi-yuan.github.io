class Scatterplot {
    ...
    initVis() {
      ...
      vis.symbolScale = d3.scaleOrdinal()
          ...
          .domain(['United States', 'United Kingdom', 'Pakistan', 'India']);
    ...
    updateVis() {
      ...
      //vis.colorValue = d => d.subscription_type;
      vis.colorValue = d => d.country;
      vis.xValue = d => d.songs_played_per_day;
      
      vis.yValue = d => d.listening_time;
    ...
    renderVis() {
      ...
      const symbols = vis.chart.selectAll('.symbol')
        ...
        .join('path')
          ...
          .attr('d', d => this.symbolScale(d.subscription_type))
          //.attr('r', 4)
          //.attr('cy', d => vis.yScale(vis.yValue(d)))
          //.attr('cx', d => vis.xScale(vis.xValue(d)))
          .attr('fill', d => vis.config.colorScale(vis.colorValue(d)));
      ...
      symbols
          .on('mouseover', (event,d) => {
            d3.select('#tooltip')
              ...
              .html(`
                <div class="tooltip-title">${d.subscription_type}</div>
                <div><i>${d.age}</i></div>
                <ul>
                  <li>${d.age}, ~${d.listening_time} minutes</li>
                  <li>${d.subscription_type}</li>
                  <li>${d.gender}</li>
                </ul>
              `);
          })
