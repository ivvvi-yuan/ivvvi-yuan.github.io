...
d3.csv('data/spotify_new_data.csv')
   .then(_data => {
     ...
     const diffLevel = data.map(d => d.country);
     
     const colorScale = d3.scaleOrdinal()
                            //.domain(['Free', 'Student', 'Family', 'Premium'])
                            .domain(['United States', 'United Kingdom', 'Pakistan', 'India', 'Australia', 'France', 'Germany', 'Canada'])
                            .range(['palegreen', 'mediumseagreen','red', 'green', 'yellow', 'purple', 'blue', 'black']);//, 'red','green']);
...
dispatcher.on('filterCategories', selectedCategories => {
	if (selectedCategories.length == 0) {
		...
		scatterplot.data = data.filter(d => selectedCategories.includes(d.country));
	}
	scatterplot.updateVis();
});
