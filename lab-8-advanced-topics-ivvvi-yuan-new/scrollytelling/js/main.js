...
d3.csv('data/assignments.csv').then(_data => {
  ...
  d3.select('#intermediate-assignments-count').text(data.filter(d => d.difficulty == 'Intermediate').length);
  ...
  d3.select('#max-duration-assginment').text(longestAssignment.name);
